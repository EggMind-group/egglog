...
impl ProofStore {
    /// Collect rule names from a term proof in dependency order.
    pub fn term_rule_trace(&self, term_pf: TermProofId) -> Vec<String> {
        let mut out = Vec::new();
        self.collect_term_rule_trace(term_pf, &mut out);
        out
    }

    /// Collect rule names from an equality proof in dependency order.
    pub fn eq_rule_trace(&self, eq_pf: EqProofId) -> Vec<String> {
        let mut out = Vec::new();
        self.collect_eq_rule_trace(eq_pf, &mut out);
        out
    }

    fn collect_term_rule_trace(&self, term_pf: TermProofId, out: &mut Vec<String>) {
        ...
        match term_pf {
            TermProof::PRule {
                ...
                for premise in body_pfs {
                    self.collect_premise_rule_trace(*premise, out);
                }
                out.push(rule_name.to_string());
            }
            TermProof::PProj { pf_f_args_ok, .. } => {
                self.collect_term_rule_trace(*pf_f_args_ok, out);
            }
            TermProof::PCong(cong_pf) => {
                for pf in &cong_pf.pf_args_eq {
                    self.collect_eq_rule_trace(*pf, out);
                }
                self.collect_term_rule_trace(cong_pf.pf_f_args_ok, out);
            }
            TermProof::PFiat { .. } => {}
        }
    }

    fn collect_eq_rule_trace(&self, eq_pf: EqProofId, out: &mut Vec<String>) {
        ...
        match eq_pf {
            ...
            EqProof::PRefl { t_ok_pf, .. } => {
                self.collect_term_rule_trace(*t_ok_pf, out);
            }
            EqProof::PSym { eq_pf } => {
                self.collect_eq_rule_trace(*eq_pf, out);
            }
            EqProof::PTrans { pfxy, pfyz } => {
                self.collect_eq_rule_trace(*pfxy, out);
                self.collect_eq_rule_trace(*pfyz, out);
            }
    ...
    fn collect_premise_rule_trace(&self, premise: Premise, out: &mut Vec<String>) {
        match premise {
            Premise::TermOk(term_pf) => self.collect_term_rule_trace(term_pf, out),
            Premise::Eq(eq_pf) => self.collect_eq_rule_trace(eq_pf, out),
        }
    }
